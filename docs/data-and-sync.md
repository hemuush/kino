...
3. Otherwise read `kino-index.json` to learn `chunkCount`, then issue **every** `kino-chunk-i.json` and `kino-images-i.json` request at once (two arrays of in-flight promises) — only the *processing* of results stays sequential (`await`ed in order, chunk 0 first) so `onChunkLoaded` still fires in order and the UI can render chunk 0 the moment it's ready. This is what actually matters for "fast loading": a library with several chunks used to pay each chunk's round-trip latency one at a time; starting them together collapses that to roughly one round-trip regardless of chunk count. Image chunks are additionally hydrated in the background (`Promise.all`'d at the end) since they aren't needed to render the list, only to paint posters.
