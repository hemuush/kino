...
- **`Badge.tsx`** — small pill label with a fixed set of semantic variants (`primary`/`secondary`/`success`/`warning`/`accent`/`muted`/`movie`/`tv`/`anime`), each a pre-set color/border/background combo. Used in `MediaDetailModal`'s header for type/status indicators.
