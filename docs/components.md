...
The full-screen "Wrapped"-style animated slideshow, rendered by `/wraps?type=weekly|monthly` (see [docs/pages.md](pages.md)). Four slides (`intro` → `time` → `genre` → `top`), each auto-advancing after 12 seconds via a per-slide progress bar (`onAnimationComplete` on the active bar's width animation calls `nextSlide()`); tapping the left third of the screen goes back, the right two-thirds advances. All four rely on the same `stats` `useMemo`, which filters `entries` to those `updatedAt`/`createdAt` within the last 7 or 30 days, sums `getWatchedRuntimeMinutes` across them, tallies type/genre counts, and picks the highest-rated recent entry as the "highlight."
