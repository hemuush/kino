...
- **Weekly/monthly recap auto-launch**: on load, if it's the first Sunday-load of the week (or first-of-month, checked via `localStorage['kino_last_weekly_recap'/'kino_last_monthly_recap']`), automatically redirects into `/wraps?type=weekly|monthly` once per period. (`isEarlyMonth`/`isSunday` are derived from the same deferred `nowTimestamp` state as the Release Calendar's future/past split — never a bare `new Date()` at render.)
- **On This Day**: an `onThisDay` `useMemo` scans `entries` for anything whose `updatedAt`/`createdAt` falls on today's exact month+day in a past year, preferring `Completed` entries, then highest rating, then the most recent past year if several tie. Shown as a small clickable card (opens that entry in `MediaDetailModal`) next to a **Quick Note** widget — a minimal `QuickJournalNote` component (its own `useMedia()` call, not wired through `DashboardContent`'s props) that calls `addJournalEntry` directly so a thought can be captured without leaving the dashboard for Profile → Journal.
...
- **Floating segmented dock** (bottom of screen, Apple/Nothing-style): type filter, poster/list toggle, an Add shortcut, and a filter-popover toggle for status/sort/genre/saga — kept separate from the Cmd+K palette since the dock is meant for quick repeated taps, the palette for keyboard-first power use.

## `/media/[id]` — `src/app/media/[id]/page.tsx`

The full-page detail view (as opposed to `MediaDetailModal`, the sheet/modal opened from a card). `Completed` entries get a **Rewatches** card (count + "Log Rewatch", via the shared `incrementRewatch` helper — see [docs/data-and-sync.md](data-and-sync.md)) right above Review & Notes; `MediaDetailModal` has the identical card in the same spot, so logging a rewatch works the same from either surface. Built around **per-field quick-edit**: nearly every displayed value (title, release date, runtime, cover image, review, saga, genres, episode total) is independently clickable and opens a small centered modal to edit *just that field*, tracked via a single `editingField` union-typed state rather than one state variable per field. Status and rating each get their own dedicated modal (`editingStatus`/`editingRating`) since they need a fixed choice list / star picker rather than a text input.
...
- **`DataManager` (Data & Cloud)** — sync status banner, Drive size/chunk-count display (via `getBackupMetadataFromDrive`), the **account migration** flow (`handleExport` downloads `{ entries, genres, franchises, journal, exportedAt }` as a JSON file — this is the file a user takes to a different Google account), and the Danger Zone (wipe, behind a hard `window.confirm()` — the one deliberate exception to this app's "no `alert()`/`confirm()`" rule). The raw per-file chunk breakdown, a "Force Restructure Data" button, and the recovery panel below all sit behind a collapsed **Advanced** disclosure (`showAdvanced` state) — internal-architecture detail most users never need to see.
  - **`RecoveryPanel`** (`src/components/settings/RecoveryPanel.tsx`) — emergency-only tool added after a real data-loss incident (see the safety guards documented in [docs/data-and-sync.md](data-and-sync.md)). Lists prior Drive revisions of `kino-index.json` (`listFileRevisions`/`getRevisionContent` in `googleDrive.ts`) — since every sync overwrites that file via `files.update`, Drive typically keeps its revision history, so a bad overwrite is often still recoverable there. Lets the user preview a revision's genre/saga/journal counts before merging it back in via the existing safe `importData` path (never a raw overwrite). Explicit in its own copy that it **cannot** recover deleted entry chunk files — those are gone via `files.delete`, which bypasses trash and takes any revision history with it.
...
- **Achievements** (`src/components/profile/AchievementsManager.tsx`) — two badge sets, both purely derived (nothing about badge state is persisted):
  - A fixed list (`b.push({ id, name, desc, unlocked, icon, color })`), each `unlocked` computed inline from `entries` (counts of completed movies/shows/anime, perfect/high ratings, distinct genres used, favorites, franchise usage, total completions).
  - **Saga Completions** — a `sagaBadges` `useMemo` that generates one badge **per franchise the user has tagged** (2+ titles required), unlocked once every entry with that `franchiseId` is `Completed`. This is deliberately dynamic rather than a hardcoded list of franchise names (e.g. "Harry Potter", "Marvel") — it works for whatever a user actually names their Sagas, with no special-casing needed.
  - Also hosts the recap launchers: a full-width flagship "Yearly Wrap" card above a Weekly/Monthly pair, all pushing to `/wraps?type=...`.
