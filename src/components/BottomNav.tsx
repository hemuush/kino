    ...
    <>
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-background/80 dark:bg-[#0c0c0d]/90 backdrop-blur-3xl border-t border-border/40 safe-area-bottom pb-3 pt-1">
        <div className="flex items-center justify-between px-2 h-[65px] max-w-md mx-auto relative">
          ...
          <Link href="/dashboard" aria-label="Dashboard" className={`flex flex-col items-center justify-center gap-1 flex-1 ${pathname === '/dashboard' || pathname === '/' ? 'text-primary' : 'text-muted-foreground'}`}>
            <LayoutDashboard size={24} strokeWidth={pathname === '/dashboard' || pathname === '/' ? 2.5 : 1.8} />
          </Link>

          <Link href="/collection" aria-label="Collection" className={`flex flex-col items-center justify-center gap-1 flex-1 ${pathname === '/collection' ? 'text-primary' : 'text-muted-foreground'}`}>
          ...
          <Link href="/sagas" aria-label="Sagas" className={`flex flex-col items-center justify-center gap-1 flex-1 ${pathname === '/sagas' ? 'text-primary' : 'text-muted-foreground'}`}>
            <Film size={24} strokeWidth={pathname === '/sagas' ? 2.5 : 1.8} />
          </Link>

          <Link href="/settings" aria-label="Settings" className={`flex flex-col items-center justify-center gap-1 flex-1 ${pathname === '/settings' ? 'text-primary' : 'text-muted-foreground'}`}>
