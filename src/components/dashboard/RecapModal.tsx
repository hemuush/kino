...
export function RecapModal({ isOpen, onClose, entries, genres, type }: RecapModalProps) {
  ...
  // 2. Dynamic Ambient Backgrounds based on active slide
  const backgroundGradients = [
    "from-zinc-900 via-zinc-950 to-black", // Intro - Neutral Dark
    "from-blue-900/40 via-zinc-950 to-black", // Time - Blue
    "from-purple-900/40 via-zinc-950 to-black", // Genre - Purple
    "from-amber-900/40 via-zinc-950 to-black", // Top - Gold
  ];
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col items-center text-center space-y-6 w-full">
          <StaggeredText text={`Highlight of the ${type === 'weekly' ? 'Week' : 'Month'}`} className="text-3xl font-display font-bold" />
          {stats.topEntry ? (
            <motion.div 
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="text-muted-foreground mt-10">No highly rated entries this {type}.</motion.p>
          )}
        </div>
      )
    }
