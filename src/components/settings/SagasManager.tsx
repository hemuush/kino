    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <form onSubmit={addNewFranchise} className="flex flex-col sm:flex-row gap-3 mb-8">
                <input
                    type="text"
                    placeholder="Enter new saga name..."
                    value={newValue}
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-3 hide-scrollbar pb-4 min-h-[300px]">
                <AnimatePresence>
                    {franchises.length === 0 && (
                        ...
                        >
                            <p className="text-sm text-muted-foreground font-medium">No sagas created yet.</p>
                        </motion.div>
                    )}
                        ...
                        >
                            {editingId === f.id ? (
                                <input 
                                    ...
                                    onKeyDown={(e) => e.key === 'Enter' && saveFranchise(f.id)} 
                                ...
                                <span className="text-[15px] font-semibold text-foreground truncate pr-4">{f.name}</span>
                            )}

                            <div className="flex items-center gap-2 shrink-0">
                                {editingId === f.id ? (
                                    <>
                                        <button onClick={() => saveFranchise(f.id)} className="p-2 text-green-500 hover:bg-green-500/10 rounded-xl transition-colors"><Check size={18} /></button>
                                    ...
                                    <>
                                        <button onClick={() => startEdit(f)} className="p-2 text-muted-foreground hover:text-foreground hover:bg-background rounded-xl transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100"><Edit2 size={18} /></button>
                                        <button onClick={() => setFranchises(franchises.filter(x => x.id !== f.id))} className="p-2 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100"><Trash2 size={18} /></button>
                                    </>
                                )}
                            </div>
                        </motion.div>
