...
* **No favorite/like feature — deliberately removed, don't add it back.** `MediaEntry` has no `favorite` field. It existed earlier and was removed at the user's explicit request; Profile's "Hall of Fame" now shows highest-rated titles (by `rating`) instead. If a future request wants a "mark as special" concept, treat it as a new feature to discuss, not a revert of this one.
